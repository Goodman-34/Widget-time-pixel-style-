...
**Widget jam pixel-art untuk Web dan desktop Windows.**
...
[![Platform](https://img.shields.io/badge/platform-Web%20%7C%20Windows-0078D6)](#-kebutuhan-sistem)
...
> code (no image files), the entire asset payload is ~145 KB, and it idles at
> ~0.5% CPU. Click the truck for a surprise. Requirements are in
> [§ Kebutuhan sistem](#-kebutuhan-sistem); a legacy Windows 7 build is
> included with each release. The rest of this document is in Indonesian.
...
- **Mobil lalu lintas** — pickup merah/biru/hijau sesekali melintas dari arah
  berlawanan, lampunya ikut menyala kalau hari sudah gelap.
- **Bayangan mobil** ikut arah dan ketinggian matahari.

### Iseng-iseng: klik mobilnya 😉

Arahkan kursor ke mobil (bentuknya berubah jadi telunjuk) lalu klik:

1. Kaca samping turun, pengemudi menoleh dan **berkedip** ke Anda.
2. Kaca naik lagi, lalu **nitro menyala** — semburan api di knalpot dan dunia
   melaju kencang.
3. Nitronya kelewat batas: **mesin mogok**. Mobil berhenti, kap terbuka
   mengepulkan uap, dan pengemudi turun untuk memeriksa serta memperbaikinya.
4. Setelah **30–100 detik** mesinnya sehat lagi, pengemudi kembali masuk, dan
   mobil melaju seperti biasa.

Selama animasi dan perbaikan berlangsung, mobil **tidak bisa diklik lagi** —
tunggu pengemudinya selesai dulu.
...
|---|---|---|
| **Sistem operasi** | **Windows 10 64-bit** atau lebih baru | Untuk Windows 7/8/8.1 tersedia unduhan terpisah — lihat [Versi legacy Windows 7](#versi-legacy-windows-7). |
...
### Pemakaian sumber daya (terukur, v1.0.1)

| Metrik | Nilai |
|---|---|
| RAM privat | **±101 MB** |
| RAM working set | ±230 MB |
| CPU saat tidak difokuskan | **±0,5 %** dari 8 core |
| Jumlah proses | **3** (proses GPU digabung ke proses utama) |
| Ukuran terpasang di disk | ±301 MB |
| Ukuran installer | 87 MB |
| **Kode + aset widget sendiri** | **±145 KB** |
...
### Versi legacy Windows 7

Rilis utama memakai Electron 43, yang **tidak bisa berjalan** di Windows
7/8/8.1 (dukungannya dihapus Chromium sejak 2023 — ini batas teknis, bukan
pilihan). Karena itu setiap rilis juga menyertakan
**`PixelDriveClock-x.y.z-legacy-win7.exe`**: versi **portable** berbasis
Electron 22.3.27, versi terakhir yang masih mendukung Windows 7 (mencakup juga
Windows 8/8.1). RAM 4 GB tetap lega — widget ini hanya memakai ±100 MB.

Dua hal yang perlu Anda tahu, apa adanya:

- **Chromium 108 di dalamnya sudah tidak menerima patch keamanan** (berakhir
  akhir 2023). Untuk widget ini risikonya kecil karena ia 100% offline — tidak
  pernah memuat apa pun dari jaringan dan semua navigasi ditolak — tapi tetap
  perlu dinyatakan jujur.
- Versi legacy **belum diuji di Windows 7 sungguhan**. Yang sudah diverifikasi:
  aplikasi ini berjalan normal dengan runtime Electron 22 (jendela, animasi,
  mode tangkap). Kalau bermasalah di Windows 7 asli, laporkan lewat Issues.
...
### 🌐 Rilis Web Ringan (1.0.2 - Terbaru [DALAM TAHAP PENGEMBANGAN])

versi 1.0.2, aplikasi ini dirombak menjadi web widget statis untuk performa terbaik dan tanpa proses instalasi raksasa. Untuk saat ini, masih belum dapat di install dan dalam proses pengembangan.
...
| Tindakan | Caranya |
|---|---|
| **Memindahkan** | Seret langsung gambarnya. Saat panel pengaturan terbuka, seret **kepala panel** (tulisan PENGATURAN) |
| **Klik mobil** | Klik mobilnya saat kursor berbentuk telunjuk — lihat [Iseng-iseng](#-fitur) |
...
6. **Mode hemat RAM** mematikan akselerasi hardware **dan menggabungkan proses
   GPU ke proses utama**. Karena seluruh gambar memang dihitung di CPU dan
   tidak ada WebGL sama sekali, proses GPU Chromium nyaris tidak bekerja tapi
   tetap memakan RAM. Keduanya terukur menghemat **±49 MB RAM privat tanpa
   menambah beban CPU** — dan widget jadi hanya 3 proses, bukan 4.
...
| Perintah | Fungsi |
|---|---|
| `npm start` | Menjalankan *local server* (melalui modul serve) dan membuka widget web |
...
```
src/
  main.js               (Hanya v1.0.1) proses utama Electron: jendela, tray, setelan, IPC
  preload.js            (Hanya v1.0.1) jembatan aman renderer <-> main (tanpa akses Node)
...
release/                (Sejak v1.0.2) ZIP File dari rilis web yang siap digunakan
```
